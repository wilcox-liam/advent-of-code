	...
	for k := 0; k < len(lines[0]); k++ {
		for l := 0; l < len(lines[1]); l++ {
			if lines[0][k] == lines[1][l] {
				distance := l + k + 2 //Because arrays index at 0. +1 for each array
				fmt.Println(lines[0][k], k, l, distance)
...
func calculate_points_between_coords(origin Coordinate, target Coordinate) []Coordinate {
	if origin.X == target.X {
		if origin.Y > target.Y {
			return calculate_points_y_axis(origin.Y, target.Y, origin.X, -1)
		} else {
			return calculate_points_y_axis(origin.Y, target.Y, origin.X, 1)
		}
	} else if origin.Y == target.Y {
		if origin.X > target.X {
			return calculate_points_x_axis(origin.X, target.X, origin.Y, -1)
		} else {
			return calculate_points_x_axis(origin.X, target.X, origin.Y, +1)
		}
	} else {
...
func calculate_points_x_axis(origin int, target int, y int, direction int) []Coordinate {
	if origin == target {
		log.Fatal()
	}

	var points []Coordinate
	for origin += direction; origin != target; origin += direction {
...
func calculate_points_y_axis(origin int, target int, x int, direction int) []Coordinate {
	if origin == target {
		log.Fatal()
	}
